...
return (
<>
{showSuccess && ( <div className="donor-donation-success-toast">Donation deleted successfully!</div>
...
{deleteModal.open && (
  ...
  <div className="donor-donation-modal-overlay">
    <div className="donor-donation-modal-content">
      ...
      <p className="donor-donation-modal-message">Are you sure you want to delete this donation? This action cannot be undone.</p>
      ...
      <div className="donor-donation-my-donation-container">
        ...
        {selectedDonation && selectedDonation.status !== "Cancel" && (
          <div className="donor-donation-donation-workflow-wrapper">
            <h3 className="donor-donation-donation-workflow-title">Donation Workflow</h3>
            <div className="donor-donation-donation-steps-container">
              {["Pending", "Collected", "Packaging", "Delivery", "Completed"].map((status, index) => (
                <React.Fragment key={status}>
                  <div className={`donor-donation-donation-status-step ${getActiveStep(selectedDonation.status) >= index ? "donor-donation-active" : ""}`}>
                    <div className="donor-donation-donation-step-number">{index + 1}</div>
                    <div className="donor-donation-donation-step-name">{status}</div>
                  </div>
                  {index < 4 && (
                    <div className={`donor-donation-donation-step-connector ${getActiveStep(selectedDonation.status) > index ? "donor-donation-active" : ""}`}>
                      <span className="donor-donation-donation-arrow">→</span>
                    </div>
                  )}
                </React.Fragment>
              ))}
