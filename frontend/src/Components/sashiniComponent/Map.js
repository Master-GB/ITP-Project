...
const Map = () => {
  ...
  return (
    <div className="map-container">
          ...
          {showModeSelector && (
            <div className="mode-options">
              {travelModes.map(mode => (
                <button
                  key={mode.value}
                  className={`mode-option ${travelMode === mode.value ? 'active' : ''}`}
                  onClick={() => {
      ...
      <LoadScript
        googleMapsApiKey={process.env.REACT_APP_GOOGLE_MAPS_API_KEY || "AIzaSyD9KK3Rb0VUF5WEcN6M7VuejWlmvKOqFzo"}
        ...
        <ul>
          {needyLocations.map((location) => (
            <li 
              ...
              className={selectedLocation?.name === location.name ? 'active' : ''}
            >
